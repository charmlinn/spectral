...
import { usePlaybackStore, useProjectStore, useTimelineStore } from "@spectral/editor-store";
import { createTimelinePropsFromVideoProject, EditorTimeline } from "@spectral/timeline";
...
export function TimelinePanel({
  ...
  const setSelectedSegmentIds = useTimelineStore((state) => state.setSelectedSegmentIds);
  ...
  const timelineProps = createTimelinePropsFromVideoProject({
    ...
    onSelectionChange: (selection) => setSelectedSegmentIds(selection.selectedSegmentIds),
    onSegmentsChange: (segments) => updateAtPath(["lyrics", "segments"], segments),
  });
    ...
    <Card className="flex min-h-[18rem] flex-col overflow-hidden">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline">Duration {Math.round(project.timing.durationMs / 1000)}s</Badge>
              <Badge variant="outline">{project.lyrics.segments.length} segments</Badge>
            </div>
            <div>
              <CardTitle>Unified timeline</CardTitle>
              <CardDescription>
                `@spectral/timeline` consumes the live project document and store-backed playback state here.
              </CardDescription>
            </div>
          </div>
      ...
      <CardContent className="min-h-0 flex-1">
        <div className="space-y-4" ref={viewportRef}>
          <EditorTimeline {...timelineProps} snapThresholdMs={snapThresholdMs} />
          <div className="rounded-[24px] border border-border/70 bg-background/50 p-4 text-sm text-muted-foreground">
            Scroll {Math.round(scrollLeft)} px · Selected segments {selectedSegmentIds.length} · Analysis{" "}
