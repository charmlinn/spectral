...
function mixToMono(audioBuffer: AudioBuffer): Float32Array {
  ...
  for (let channelIndex = 0; channelIndex < numberOfChannels; channelIndex += 1) {
    ...
    for (let sampleIndex = 0; sampleIndex < length; sampleIndex += 1) {
      mixed[sampleIndex] =
        (mixed[sampleIndex] ?? 0) + ((channel[sampleIndex] ?? 0) / numberOfChannels);
    }
  }
...
export function analyzeAudioBuffer(
  ...
  const waveform = createWaveformOverview(
    ...
    options.waveformPoints ?? Math.max(1200, Math.round(audioBuffer.duration * 240)),
  );
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    ...
    const rawWideSpectrum = byteFrequencyData.slice(
      WIDE_SPECTRUM_OPTIONS.spectrumStart,
      WIDE_SPECTRUM_OPTIONS.spectrumEnd,
    );
  ...
  const spectrumFrames: SpectrumFrame[] = rawWideSpectrums.map((rawWideSpectrum, frame) => ({
    frame,
    timeMs: Math.round((frame / fps) * 1000),
    values: createWideSpectrumFrame(rawWideSpectrum, cumulativeWideMaxMagnitude),
  }));
